...
bool CoreGex::D2::Sprites::LoadSprite(int ID,BYTE* FileName, ULONG Size )
{
	return LoadSprite(ID,FileName,Size,D3DX_DEFAULT,D3DX_DEFAULT,255,SpriteFormat::UNKNOWN);
}

bool CoreGex::D2::Sprites::LoadSprite(int ID,BYTE* FileName, ULONG Size, int width, int height)
{
	return LoadSprite(ID,FileName,Size,width,height,255,SpriteFormat::UNKNOWN);
}

bool CoreGex::D2::Sprites::LoadSprite(int ID,BYTE* FileName, ULONG Size, int width, int height,int transparentkey)
{
	return LoadSprite(ID,FileName,Size,width,height,transparentkey,SpriteFormat::UNKNOWN);
}

bool CoreGex::D2::Sprites::LoadSprite(int ID,BYTE* FileName, ULONG Size, int width, int height,int transparentkey,SpriteFormat fmt)
{
...
//	d3d->SetRenderState(D3DRS_ALPHATESTENABLE, true);
	...
	SS[ID].sprite->Begin(tri);

   // Transform(&SS[ID]);
	SS[ID].sprite->SetTransform(&SS[ID].mat);

	SS[ID].sprite->Draw(SS[ID].image,NULL,NULL,NULL,D3DCOLOR_ARGB(A,R, G, B));
...
bool CoreGex::D2::Sprites::RenderSprite(int ID,RECTANGLE^ rect,int R, int G, int B,int A)
{
	...
	// Transform(&SS[ID]);
	SS[ID].sprite->SetTransform(&SS[ID].mat);
	SS[ID].sprite->Draw(SS[ID].image,&rct,NULL,NULL,D3DCOLOR_ARGB(A,R, G, B));
